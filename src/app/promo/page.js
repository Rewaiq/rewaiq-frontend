  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A1628' }}>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <span style={{ color: '#fff', fontWeight: 600, fontSize: 16 }}>Promo Code</span>
      </div>
      ...
      <div style={{ padding: '32px 20px' }}>
        ...
        {status?.has_promo && (
          <div style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: 16, padding: '20px', marginBottom: 24, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 700, color: '#fff', marginBottom: 4 }}>Active Promo</p>
        ...
        {/* What you get */}
        <div style={{ background: '#0D1F3C', borderRadius: 16, padding: '20px', marginBottom: 24 }}>
          <p style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 14, fontFamily: 'Montserrat, sans-serif' }}>
            Beta Promo Benefits
          </p>
          {[
            ...
            <div key={item.title} style={{ display: 'flex', gap: 12, marginBottom: 14, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: '#8A9BB0', margin: 0 }}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {!success && !status?.has_promo && (
          <>
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#fff', display: 'block', marginBottom: 8 }}>Enter Promo Code</label>
              <input
                type="text"
                placeholder="e.g. REWAIQ-BETA"
                value={code}
                onChange={e => setCode(e.target.value.toUpperCase())}
                style={{ width: '100%', padding: '14px', borderRadius: 10, border: '1.5px solid rgba(255,255,255,0.1)', fontSize: 16, color: '#fff', background: 'rgba(255,255,255,0.05)', fontWeight: 700, letterSpacing: 2, textAlign: 'center', fontFamily: 'Montserrat, sans-serif' }}
              />
            </div>
        ...
        {success && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ fontSize: 18, fontWeight: 700, color: '#fff', marginBottom: 8 }}>Code Activated!</p>
            <p style={{ fontSize: 14, color: '#8A9BB0', marginBottom: 24 }}>
        ...
        <div style={{ marginTop: 28, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '18px' }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#8A9BB0', marginBottom: 12, letterSpacing: 1, textTransform: 'uppercase' }}>After Beta — Subscription Plans</p>
          {[
            { name: 'Basic', price: 'Free', desc: 'Limited streams, standard tasks', color: '#8A9BB0' },
            ...
            <div key={plan.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 700, color: '#fff', margin: 0 }}>{plan.name}</p>
                <p style={{ fontSize: 11, color: '#8A9BB0', margin: '2px 0 0' }}>{plan.desc}</p>
              </div>
              <span style={{ fontSize: 14, fontWeight: 700, color: plan.color }}>{plan.price}</span>
            </div>
          ))}
          <p style={{ fontSize: 11, color: '#8A9BB0', marginTop: 10, textAlign: 'center' }}>
