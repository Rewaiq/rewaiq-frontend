...
export default function ProfilePage() {
 ...
 const fetchProfile = async () => {
  try {
    ...
    console.log('Profile data:', profileRes.data); // ADD THIS
    const freshUser = profileRes.data.user;
    localStorage.setItem('rewaiq_user', JSON.stringify(freshUser));
    setUser(freshUser);

    try {
      const referralRes = await API.get('/api/referrals');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <span style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: 18, fontFamily: 'Montserrat, sans-serif' }}>Profile</span>
      ...
      {/* Profile card */}
      <div style={{ margin: '20px 20px 16px', background: 'var(--bg-card)', borderRadius: 20, padding: '28px 20px', textAlign: 'center' }}>
        <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'linear-gradient(135deg, #4a9eff, #1a3a8f)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 700, color: 'var(--text-secondary)', margin: '0 auto 16px', overflow: 'hidden' }}>
          {user?.profile_picture
            ? <img src={user.profile_picture} alt="profile" style={{ width: 80, height: 80, objectFit: 'cover' }} />
            : (user?.full_name?.[0] || 'U')}
        </div>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 4, fontFamily: 'Montserrat, sans-serif' }}>
          {user?.full_name || 'User'}
        </h2>
      ...
      <div style={{ display: 'flex', gap: 12, padding: '0 20px', marginBottom: 16 }}>
        {[
          ...
          const Icon = s.Icon;
          return (
            <div key={s.label} style={{ flex: 1, background: 'var(--bg-card)', borderRadius: 14, padding: '16px', textAlign: 'center' }}>
              <Icon size={22} color={s.color} style={{ marginBottom: 6 }} />
              <p style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 2, fontFamily: 'Montserrat, sans-serif' }}>{s.value}</p>
      ...
      {/* Referral code */}
      <div style={{ margin: '0 20px 16px', background: 'var(--bg-card)', borderRadius: 16, padding: '18px 20px' }}>
      ...
      <div style={{ padding: '0 20px' }}>
        {menuItems.map(({ icon: Icon, label, action, danger }) => (
          <button key={label} onClick={action}
            style={{ width: '100%', background: 'var(--bg-card)', borderRadius: 12, padding: '14px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14, border: 'none', cursor: 'pointer' }}>
