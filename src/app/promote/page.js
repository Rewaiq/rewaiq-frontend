  ...
  if (success) {
    return (
      <div style={{ minHeight: '100vh', background: '#0A1628', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 900, color: '#fff', marginBottom: 8, fontFamily: 'Montserrat, sans-serif', textAlign: 'center' }}>Payment Successful!</h2>
        <p style={{ fontSize: 14, color: '#8A9BB0', textAlign: 'center', marginBottom: 8, lineHeight: 1.7 }}>
        ...
        <button onClick={() => router.push('/home')}
          style={{ padding: '14px 32px', borderRadius: 12, background: '#4a9eff', color: '#fff', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
          Back to Home
        </button>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A1628', paddingBottom: 40 }}>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, background: '#0D1F3C', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ color: '#fff', fontWeight: 600, fontSize: 16, margin: 0 }}>Promote Your Brand</p>
          <p style={{ color: '#8A9BB0', fontSize: 11, margin: 0 }}>Step {step} of 2</p>
        </div>
      </div>
      ...
      <div style={{ padding: '20px' }}>
        ...
        {step === 1 && (
          <>
            <h3 style={{ fontSize: 20, fontWeight: 700, color: '#fff', marginBottom: 6, fontFamily: 'Montserrat, sans-serif' }}>Choose a Package</h3>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
              {[
                ...
                <div key={s.label} style={{ flex: 1, background: '#0D1F3C', borderRadius: 12, padding: '12px', textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#fff', margin: 0 }}>{s.value}</p>
                  <p style={{ fontSize: 10, color: '#8A9BB0', margin: '2px 0 0' }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            {PACKAGES.map(pkg => (
              <div key={pkg.id}
                ...
                {pkg.badge && (
                  <div style={{ position: 'absolute', top: -1, right: 16, background: pkg.badge === 'POPULAR' ? '#4a9eff' : '#D4A017', color: '#fff', fontSize: 9, fontWeight: 700, padding: '3px 10px', borderRadius: '0 0 8px 8px', letterSpacing: 1 }}>
                    {pkg.badge}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 16 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#8A9BB0' }}>{pkg.duration}</span>
          ...
          <>
            <div style={{ background: 'rgba(74,158,255,0.08)', border: '1px solid rgba(74,158,255,0.2)', borderRadius: 12, padding: '14px 16px', marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 14, color: '#fff', fontWeight: 600 }}>{pkg.name}</span>
                <span style={{ fontSize: 16, fontWeight: 700, color: '#4a9eff' }}>{pkg.display}</span>
              </div>
              <span style={{ fontSize: 12, color: '#8A9BB0' }}>{pkg.reach} · {pkg.duration}</span>
            </div>
            ...
            {[
              ...
              <div key={f.key} style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#fff', display: 'block', marginBottom: 6 }}>{f.label}</label>
                {f.key === 'task_description' ? (
                  <textarea
                    ...
                    rows={3}
                    style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid rgba(255,255,255,0.1)', fontSize: 14, color: '#fff', background: 'rgba(255,255,255,0.05)', resize: 'none', fontFamily: 'Inter, sans-serif' }}
                  />
                ) : (
                  <input
                    type={f.type || 'text'}
                    ...
                    value={form[f.key]}
                    onChange={e => setForm(prev => ({ ...prev, [f.key]: e.target.value }))}
                    style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid rgba(255,255,255,0.1)', fontSize: 14, color: '#fff', background: 'rgba(255,255,255,0.05)' }}
                  />
                )}
              </div>
            ))}
            ...
            <div style={{ background: 'rgba(37,211,102,0.08)', border: '1px solid rgba(37,211,102,0.2)', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 12, color: '#8A9BB0', marginBottom: 10 }}>Chat with us directly to set up your campaign</p>
              <button onClick={() => {
                ...
                style={{ width: '100%', padding: '11px', borderRadius: 10, background: '#25D366', color: '#fff', fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                Chat on WhatsApp
              </button>
            </div>
            ...
            <p style={{ fontSize: 11, color: '#8A9BB0', textAlign: 'center', marginTop: 10 }}>
