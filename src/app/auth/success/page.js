...
function AuthSuccessContent() {
  ...
  useEffect(() => {
    ...
    if (token) {
      localStorage.setItem('rewaiq_token', token);
      // Fetch user profile
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile`, {
        ...
        .then(data => {
          if (data.user) {
            localStorage.setItem('rewaiq_user', JSON.stringify(data.user));
            // New user goes to interests, existing goes to home
  ...
  return (
    <div style={{
      ...
      <svg width="60" height="48" viewBox="0 0 100 80">
      ...
      <p style={{ color: '#8A9BB0', fontSize: 15, fontFamily: 'Inter, sans-serif' }}>
        Signing you in...
      </p>
      <div style={{ width: 40, height: 3, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ width: '100%', height: '100%', background: '#4a9eff', animation: 'slide 1s ease infinite', borderRadius: 2 }} />
      </div>
      <style>{`
        @keyframes slide {
