...
export default function NotificationsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: 40 }}>
      {/* Header */}
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-primary)', position: 'sticky', top: 0, zIndex: 10, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => router.back()} style={{ background: 'none', display: 'flex' }}>
            <ArrowLeft size={22} color="#fff" />
          </button>
          <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 16 }}>Notifications</span>
          {unreadCount > 0 && (
            <span style={{ background: '#4a9eff', color: 'var(--text-primary)', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20 }}>
              {unreadCount}
            </span>
          )}
        </div>
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        <Spinner />
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: 8 }}>No notifications yet</p>
            <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>We'll notify you about earnings, tasks and updates</p>
          </div>
        ) : (
          notifications.map(n => (
            <div
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                  <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>{n.title}</p>
                  {!n.is_read && (
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#4a9eff', flexShrink: 0, marginTop: 4 }} />
                  )}
                </div>
                <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 6px', lineHeight: 1.5 }}>{n.message}</p>
