...
, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text-primary)', marginBottom: 8, fontFamily: 'Montserrat, sans-serif', textAlign: 'center' }}>
          Submitted for Review
        </h2>
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', textAlign: 'center', marginBottom: 8, lineHeight: 1.7, maxWidth: 280 }}>
        ...
        <div style={{ background: 'rgba(74,158,255,0.08)', border: '1px solid rgba(74,158,255,0.2)', borderRadius: 12, padding: '14px 20px', marginBottom: 28, textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '4px 0 0' }}>You will be notified when approved</p>
        </div>
...
, paddingBottom: 40 }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-card)', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 16 }}>Complete Task</span>
      </div>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: '20px', marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', margin: 0, fontFamily: 'Montserrat, sans-serif', lineHeight: 1.3 }}>
                {task.title}
              </h2>
            </div>
          ...
          {task.description && (
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.6, margin: 0 }}>{task.description}</p>
          )}
        </div>
          ...
          {getInstructions(task).map((instruction, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 10, alignItems: 'flex-start' }}>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#4a9eff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-primary)' }}>{i + 1}</span>
              </div>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.5, paddingTop: 3 }}>{instruction}</span>
            </div>
          ))}
        </div>
        ...
        {task.target_url && (
          <a href={task.target_url} target="_blank" rel="noreferrer"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '15px', borderRadius: 12, background: 'var(--bg-card)', border: '1px solid rgba(74,158,255,0.3)', color: '#4a9eff', fontSize: 15, fontWeight: 600, textDecoration: 'none', marginBottom: 20 }}>
        ...
        <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ border: `2px dashed ${proofPreview ? '#4ADE80' : 'rgba(74,158,255,0.3)'}`, borderRadius: 12, padding: '20px', textAlign: 'center', background: proofPreview ? 'rgba(74,222,128,0.04)' : 'rgba(74,158,255,0.04)', transition: 'all 0.2s' }}>
              {proofPreview ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: 0 }}>JPG or PNG — from your gallery or camera</p>
                </>
              )}
            </div>
            <input
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>OR paste a link</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
          </div>
          ...
          <input
            ...
            style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${proof && !proofPreview ? '#4a9eff' : 'rgba(255,255,255,0.1)'}`, fontSize: 14, color: 'var(--text-primary)', background: 'rgba(255,255,255,0.05)' }}
          />
        </div>
        ...
        <p style={{ fontSize: 11, color: 'var(--text-secondary)', textAlign: 'center', marginTop: 12, lineHeight: 1.6 }}>
