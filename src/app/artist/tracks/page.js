...
export default function MyTracksPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A1628', paddingBottom: 80 }}>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0D1F3C', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => router.back()} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}>
            <ArrowLeft size={22} color="#fff" />
          </button>
          <span style={{ color: '#fff', fontWeight: 600, fontSize: 16 }}>My Tracks</span>
        </div>
        <button onClick={() => router.push('/artist/upload')}
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#4a9eff', color: '#fff', padding: '8px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
          <Plus size={16} /> Upload
        </button>
      </div>
      ...
      <div style={{ padding: '16px 20px' }}>
        {tracks.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ color: '#8A9BB0', fontSize: 13, marginBottom: 24 }}>Upload your first track to start getting streams</p>
            <button onClick={() => router.push('/artist/upload')}
              style={{ padding: '13px 28px', borderRadius: 12, background: '#4a9eff', color: '#fff', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
              Upload First Track
            </button>
          </div>
        ) : tracks.map(track => {
          const progress = track.target_streams > 0
            ...
            : 0;
          return (
            <div key={track.id} style={{ background: '#0D1F3C', borderRadius: 14, padding: '16px', marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 4 }}>{track.title}</p>
                  <p style={{ fontSize: 12, color: '#8A9BB0' }}>{track.genre || 'No genre'} · {track.content_type}</p>
                </div>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#8A9BB0' }}>{track.total_streams || 0} streams</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
